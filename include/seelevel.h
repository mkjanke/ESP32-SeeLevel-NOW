...
class SeelevelInterface {
 private:
  // ESP32 Write Pin. Set HIGH to power sensors, send pulses to ground
  // to initiate sensor read
  ...
  // RMT configuration
  // Some values determined by guesswork.
  RingbufHandle_t rb = NULL;
  rmt_config_t rmt_rx_config = {.rmt_mode = RMT_MODE_RX,
 ...
 public:
  ...
  // Read a Tank level, calc checksum, convert to JSON, forward to ESP-NOW
  // Pass parameter 'tank', where
  //    tank = 1 SeeLevel tank 1 (Normally Fresh Tank)
  //    tank = 2 SeeLevel tank 2 (Normally Grey Tank)
  //    tank = 3 SeeLevel tank 3 (Normally Black Tank)
  //
  ...
  void readTank(int tank) {
    if (_xTankReadSemaphore != NULL)
      if (xSemaphoreTake(_xTankReadSemaphore, 100 / portTICK_PERIOD_MS) == pdTRUE) {
        ...
        // Send reading and checksum to ESP-NOW even if invalid.
        // Caller can decide what to do with invalid readings.
  ...
  int readLevel(int t, byte *buffer) {
    ...
    while (rb) {
      size_t rx_size = 0;
      rmt_item32_t *item =
          (rmt_item32_t *)xRingbufferReceive(rb, &rx_size, pdMS_TO_TICKS(SEELEVEL_PULSE_TIMEOUT_MILLISECONDS));
    ...
    // Delay one ms., then turn power off until next read
